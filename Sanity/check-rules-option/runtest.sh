...
TESTING_RULES="/etc/fapolicyd/rules.d/testing-${RANDOM}.rules"
...
rlJournalStart
    ...
    rlPhaseStartTest "RHEL-13011 + RHEL-169985"
        ...
        rlRun "rm -f /etc/fapolicyd/rules.d/non-existent.rules"
        rlRun -s "fapolicyd-cli --check-rules /etc/fapolicyd/rules.d/non-existent.rules" 7
        ...
        for INCOMPLETE_RULE in 'allow' \
            'allow perm' \
            'allow perm=' \
            'allow perm=any' \
            'allow perm=any exe' \
            'allow perm=any exe=' \
            'allow perm=any exe=/usr/lib/systemd/systemd' \
            'allow perm=any exe=/usr/lib/systemd/systemd :' \
            'allow perm=any exe=/usr/lib/systemd/systemd : dir' ; do
            rlRun "echo $INCOMPLETE_RULE > ${TESTING_RULES}"
            rlRun -s "fapolicyd-cli --check-rules ${TESTING_RULES}" 5
